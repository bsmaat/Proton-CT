...
void SensitiveDetector::WriteToFile(DetectorHit* aHit)
{
	...
	analysisManager->FillNtupleDColumn(4, aHit->GetMom().getX()/MeV);
	analysisManager->FillNtupleDColumn(5, aHit->GetMom().getY()/MeV);
	analysisManager->FillNtupleDColumn(6, aHit->GetMom().getZ()/MeV);
